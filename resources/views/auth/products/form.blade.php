  ...
  <form method="POST" enctype="multipart/form-data" @isset($product) action="{{ route('products.update', $product) }}"
    ...
    <div>
      ...
      {{-- <div class="input-group row">
        ...
        <div class="col-sm-6">
          ...
          <select name="property_id[]" multiple>
            ...
            <option value="{{ $property->id }}" @isset($product) @if($product->properties->contains($property->id))
              selected
              @endif
              @endisset
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <input type="checkbox" name="{{$field}}" id="{{$field}}" @if(isset($product) && $product->$field === 1)
          checked="'checked"
          @endif
          >
        </div>
